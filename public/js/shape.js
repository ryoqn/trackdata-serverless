...
export default function makeLineCoordinates(map, from, to) {
  ...
  let lineAngle =
    Math.atan2(toPointOrig.y - fromPointOrig.y, toPointOrig.x - fromPointOrig.x) + Math.PI;

  return function calcLine({ arrowSideLength, arrowSideAngle, minArrowSpacing }) {
    ...
    let lineLength = Math.sqrt((toPoint.x - fromPoint.x) ** 2 + (toPoint.y - fromPoint.y) ** 2);

    let numArrows =
      lineLength > omitArrowThreshold ? Math.max(Math.floor(lineLength / minArrowSpacing), 1) : 0;
